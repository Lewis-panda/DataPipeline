...
for category_folder in glob('classify_data/*'):
    ...
    for ds_path in glob(f'{category_folder}/first50.jsonl', recursive=True):
        name = ds_path.split("/")[-1].split(".")[0]  # Get the file name, like 'first50'
...
# Create directory for progress file if it doesn't exist
...
# Initialize the position tracking for each dataset if not already done
...
# Process each dataset and generate results
all_results = []

for category_name, content_list in dataset_map.items():
    # Get the prompt for the current category
...
#    print(prompt)
    ...
    # Process each sample in the content list
    for idx, content in enumerate(tqdm(content_list, desc=f"({model_name})Processing samples in {category_name}")):
        if idx < ds_pos.get(f"{category_name}/first50", 0):
            continue
        # Run the model with the current content
